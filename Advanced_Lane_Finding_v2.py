...
# Load the camera calibration specs which were produced from the notebook Calibrate_camera.ipynb
...
def abs_sobel_thresh(gray, orient='x', thresh_min=0, thresh_max=255, ksize= 7):
    ...
    """
    Apply a threshold based on the absolute value of the sobel filter in a certain direction.
    Sobel derivative image is rescaled to 8bit hence the threshold should be in [0, 255]
...
class LaneFinder(object):
    ...
    def _fit_lanes(self, image):
        ...
        if left_lane.detected and right_lane.detected:
            left_fit, right_fit, yvals, left_fitx, right_fitx =search_around_previous_fit(image, left_lane.best_fit, right_lane.best_fit)
        else:
...
if __name__ == "__main__":
    ...
    LF = LaneFinder()
    white_output = 'output_video1.mp4'
